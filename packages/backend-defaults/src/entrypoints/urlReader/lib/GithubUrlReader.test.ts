...
'bar',
{status: 200,
headers: {"Etag":"foo","Last-Modified":"ew Date('2021-01-01T00:00:00Z').toUTCString("},
});
          },
        ),
      );
...
headers: {"Content-Type":"application/json"},
        ...
        http.get(
          ...
          ({request}) => {
 let req = request;
            if (new URL(req.url).searchParams.get('per_page') === '0') {
              return HttpResponse.json(
commitStatusGithubResponse,
{status: 200,
headers: {"Content-Type":"application/json"},
});
            }
...
repoBuffer,
{status: 200,
headers: {"Content-Type":"application/x-gzip","content-disposition":"attachment; filename=backstage-mock-etag123.tar.gz"},
})},
        ),
        http.get(
